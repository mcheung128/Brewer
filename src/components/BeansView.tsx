...
function BeansView({ calcRatio, formatDate, onAddBeans, onDeleteBean, onUpdateBean, state }: BeansViewProps) {
  ...
  const updateEditDraft = <K extends keyof NewBeanDraft>(key: K, value: NewBeanDraft[K]) => {
                ...
                <article key={bean.id} className="bean-card">
                  ...
                  <dl>
                    ...
                    <div>
                      ...
                      <dd>{bestBrew ? `${bestBrew.method} - 1:${calcRatio(bestBrew.dose, bestBrew.water)}` : "None yet"}</dd>
                    </div>
                    <div>
                      <dt>Bag notes</dt>
                  ...
                  {editingBeanId === bean.id && editDraft ? (
                    <div className="subpanel bean-editor">
                      <div className="form-grid">
                        <label>
                          Roaster
                          <input value={editDraft.roaster} onChange={(event) => updateEditDraft("roaster", event.target.value)} />
                        </label>
                        <label>
                          Coffee name
                          <input value={editDraft.coffeeName} onChange={(event) => updateEditDraft("coffeeName", event.target.value)} />
                        </label>
                        <label>
                          Origin / country
                          <input value={editDraft.originCountry} onChange={(event) => updateEditDraft("originCountry", event.target.value)} />
                        </label>
                        <label>
                          Region / farm
                          <input value={editDraft.regionFarm} onChange={(event) => updateEditDraft("regionFarm", event.target.value)} />
                        </label>
                        <label>
                          Variety
                          <input value={editDraft.variety} onChange={(event) => updateEditDraft("variety", event.target.value)} />
                        </label>
                        <label>
                          Process
                          <input value={editDraft.process} onChange={(event) => updateEditDraft("process", event.target.value)} />
                        </label>
                        <label>
                          Roast level
                          <input value={editDraft.roastLevel} onChange={(event) => updateEditDraft("roastLevel", event.target.value)} />
                        </label>
                        <label>
                          Roast date
                          <input type="date" value={editDraft.roastDate} onChange={(event) => updateEditDraft("roastDate", event.target.value)} />
                        </label>
                        <label>
                          Bag size
                          <input value={editDraft.bagSize} onChange={(event) => updateEditDraft("bagSize", event.target.value)} />
                        </label>
                        <label>
                          Price
                          <input value={editDraft.price} onChange={(event) => updateEditDraft("price", event.target.value)} />
                        </label>
                        <label>
                          Grinder pairing
                          <input value={editDraft.grinderPairing} onChange={(event) => updateEditDraft("grinderPairing", event.target.value)} />
                        </label>
                        <label className="full-span">
                          Bag notes
                          <textarea rows={3} value={editDraft.bagNotes} onChange={(event) => updateEditDraft("bagNotes", event.target.value)} />
                        </label>
                      </div>
                      <div className="inline-actions">
                        <button className="secondary-button" onClick={stopEditing}>
                          Cancel
                        </button>
                        <button
